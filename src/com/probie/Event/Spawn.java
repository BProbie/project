...
public class Spawn {
    public static void function(String title,String value) {
        ...
        for (int i = 10; i<10+values.length*Integer.parseInt(Objects.requireNonNull(Data.getData("functionSpeed"))); i=i+Integer.parseInt(Objects.requireNonNull(Data.getData("functionSpeed")))) {
            switch (values[(i-10)/Integer.parseInt(Objects.requireNonNull(Data.getData("functionSpeed")))]) {
                ...
                case "2↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._RE; break;
                case "3↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._MI; break;
                case "4↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._FA; break;
                case "5↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._SO; break;
                case "6↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._LA; break;
                case "7↓": function = function+"\n"+"execute @a[score_"+title+"_min="+i+",score_"+title+"="+i+"] ~ ~ ~ playsound minecraft:block.note.harp music @a ~ ~ ~ 9 "+
                        Data._SI; break;
